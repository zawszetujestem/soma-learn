...
1. **Sprawdź stan kamienia milowego** (tanie, nie wymaga jeszcze pliku referencyjnego):
...
3. **Jeśli kamień milowy jest otwarty, a użytkownik poprosił o ponowne wygenerowanie dekompozycji** (lub przekazał argument ścieżki źródłowej, np. `/10x-roadmap @path/to/prd.md`), pomiń plik referencyjny: przechwyć ścieżkę (usuń początkowe `@`), w przeciwnym razie domyślnie `context/foundation/prd.md` i przejdź bezpośrednio do Kroku 1. Regeneracja zachowuje frontmatter kamienia milowego i `## Milestone History` dosłownie i przenosi statusy elementów za pomocą `Change ID` (tylko do przodu).
...
Jeśli plik istnieje, **przeczytaj go W CAŁOŚCI** (bez `limit`/`offset`). Jeśli użytkownik wybrał samodzielny opis, przechwyć jego opis dosłownie — staje się on kartą `## Milestone` z numerowanymi kotwicami zakresu `MS-NN`, a sprawdzenie gotowości PRD w Kroku 3 zostaje zastąpione sprawdzeniem kotwicy (< 2 destylowalnych kotwic `MS-NN` → poproś użytkownika o uściślenie opisu, a następnie ZATRZYMAJ, jeśli nie może).

Jeśli nazwany plik nie istnieje, zapytaj za pomocą wybranego narzędzia do zadawania pytań interaktywnych:

Zapytaj użytkownika: "Nie znaleziono źródła pod `<resolved-path>`. Jak chcesz postąpić?"
Opcje:
...
4. **Logika biznesowa wypełniona** — pierwsza niepusta linia sekcji `## Business Logic` jest zdaniem deklaratywnym (nie `# TODO: domain rule`).
...
**Wynik < 3**: ostrzeż jawnie. Nazwij, czego brakuje i dlaczego jest to ważne dla mapy drogowej (NIE ogólne "twoje PRD jest słabe"):

```
To PRD uzyskało <N>/4 w heurystyce gotowości mapy drogowej. Brakuje sygnałów:
...
uściślić PRD, wynikowa mapa drogowa będzie znacznie bardziej użyteczna.
```
...
Opcje:
- Najpierw uściślij PRD (Zalecane)
...
W przypadku "Najpierw uściślij PRD": wydrukuj przekierowanie i ZATRZYMAJ. W przypadku "Kontynuuj mimo to": kontynuuj z zapisanym wynikiem, aby Krok 6 mógł oznaczyć słabe obszary.
...
**Warstwy do sondowania** (pomiń warstwę, jeśli `tech-stack.md` już nazywa wybór tej warstwy — zamiast sondowania zgłoś "zgodnie z tech-stack.md: <wybór>"):
...
**Uruchom wszystkie sondy w jednym wywołaniu wsadowym, gdy host to obsługuje.** Każdy monit jest krótki i samodzielny; delegowani agenci zwracają tylko jeden akapit każdy, więc główny kontekst pozostaje mały. Przykład dla uwierzytelniania:
...
Zapytaj użytkownika: "Czy ta baza odpowiada Twojemu rozumieniu? Coś do poprawienia lub dodania, zanim zostanie wykorzystane w Foundations?"
Opcje:
...
- Dodaj coś, czego nie ma na liście
  Opis: Swobodny. Rzeczy, które sondy przeoczyły (zaplanowane, ale niepodłączone, szkielet z innego repozytorium itp.).
...
- **Jeśli tylko jedna wartość jest wiarygodna** (5a nie znalazło rozsądnej alternatywy), przedstaw tylko rekomendację i "Coś innego — wyjaśnię" i ujawnij w tekście pytania: "artefakty wspierają tylko jedną interpretację; zgłoś, jeśli twoja interpretacja jest inna".
...
Wyślij pojedynczą wiadomość w formacie markdown, która zamyka ramy. Bez nowych pytań. Odzwierciedlaj język użytkownika od początku do końca (polskie PRD → polskie podsumowanie). Kształt:

```markdown
Ustalanie ram mapy drogowej:
...
Gdy użytkownik powie "go" lub pozostanie cichy po przekroczeniu następnego kroku, kontynuuj z zablokowanymi ramami. Nadpisania poszczególnych linii są akceptowane i ponownie zapisywane bez ponownego pytania o inne kotwice.
...
1. **Otwórz wywiad, ujawniając to** w wiadomości poprzedzającej pierwsze pytanie kotwiczące: *"To PRD nie pasuje do znanego wzorca MVP (brak pulpitu SaaS / CRUD / treści / kształtu wrappera AI). Moje rekomendacje dla kolejnych 2-3 pytań są słabsze niż zwykle — mocno sprzeciwiaj się, jeśli moja interpretacja jest błędna."*
2. **Złagodź rekomendację dla `north_star` i wszelkich wywodzonych obszarów inwestycji.** Sformułuj opis rekomendacji jako *"Moja najlepsza interpretacja to X, ale sygnał artefaktu jest słaby"* zamiast *"PRD §Vision mówi X"*.
...
- **Cytaty w opisach opcji zasługują na swoje miejsce.** Cytat, taki jak *"tech-stack wskazuje Astro + Supabase + OpenRouter"*, jest listą nazw, chyba że następna klauzula mówi, dlaczego jest to ważne dla *tej* kotwicy. Albo włącz implikację, albo usuń cytat.
...
**M-<seq>: <Nazwa kamienia milowego>** — Status: open
...
krótkim zdaniu w prostym języku. Przykład: "Klin produktu — jedyna cecha,
która, jeśli zostanie usunięta, sprawia, że produkt jest nie do odróżnienia
od ogólnego narzędzia AI — polega na tym, że karty muszą być zarówno
oparte na AI w tekście wklejonym przez uczącego się, jak i zatwierdzone
przez człowieka, zanim trafią do talii." Czytelnik, który nie przeszedł
kursu strategii produktu, musi być w stanie przeczytać sekcję od razu.>
...
> tylko wtedy, gdy to działa. Dołącz to wyjaśnienie za PIERWSZYM razem,
> gdy "gwiazda północna" pojawi się w treści dokumentu; nie powtarzaj go później.
...
- **<Element>** — Dlaczego zaparkowany: <odnośnik do PRD §Non-Goals lub uzasadnienie z wywiadu>.
- ...
...
- **Wynik** jest prowadzony czasownikiem. Fragmenty: *"użytkownik może się zalogować i zobaczyć pustą lodówkę"*. Fundamenty: *"(fundament) szkielet uwierzytelniania wylądował; tokeny wydane za pośrednictwem skonfigurowanego dostawcy"*. Nigdy fraza rzeczownikowa ("system uwierzytelniania"); zawsze deklaracja stanu rzeczy.
...
- **Odblokowuje** pojawia się tylko w Fundamentach. Nazywa powód niższego szczebla istnienia tego Fundamentu: konkretne fragmenty `S-NN`, blokujące niewiadome lub ścieżki weryfikacji. Fundament bez Odblokowań to dryf poziomy.
...
- **Ryzyko** to jedna linia: dlaczego sekwencjonowane tutaj, co może pójść nie tak, dlaczego jest to bezpieczniejsza kolejność niż alternatywy. Nie jest to analiza pośmiertna. Nie jest to katastrofizowanie. Tylko kluczowy powód, który przyszły czytelnik musi zrozumieć sekwencję.
- **Status** cykl życia: `proposed` (domyślny przy pierwszym generowaniu) | `ready` (wszystkie wymagania wstępne spełnione, brak blokujących niewiadomych — `/10x-plan` może działać) | `planning` | `in-progress` | `done` | `blocked` (jedna lub więcej niewiadomych z `Block: yes`). Ta umiejętność emituje tylko `proposed`, `ready` i `blocked` przy generowaniu; reszta jest zapisywana w dół (patrz "Relacja z innymi umiejętnościami"), z najlepszym wysiłkiem i tylko do przodu.
...
**Brak jednostek czasu. Brak szacunków. Brak ocen złożoności.** (Zasada 5.) Kolejność jest zakodowana w wymaganiach wstępnych; tempo w blokerach i niewiadomych. Chęć napisania "to powinno zająć kilka godzin" oznacza, że dryfowałeś w terytorium `/10x-plan` — zatrzymaj się.
...
6. **Parzystość tabeli "W skrócie"** — wiersze tabeli odpowiadają treści sekcji. `Change ID`, `Prerequisites`, `PRD refs`, `Status` każdego wiersza odpowiadają dosłownie polom treści.
...
16. **Integralność kamienia milowego** — `milestone_status` jest `open` przy generowaniu; `milestone_seq` jest o 1 większe niż najwyższe zamknięte `M-<seq>` w `## Milestone History` (1, gdy historia jest pusta); karta `## Milestone` `M-<seq>` pasuje do `milestone_seq`; każda kotwica `MS-NN` odwołująca się do dowolnego fragmentu istnieje w karcie; `## Milestone History` została przeniesiona dosłownie (nigdy nie edytowana, nigdy nie obcinana) przy regeneracji i otwieraniu następnego kamienia milowego.
...
Jeśli plik istnieje, konwencja dokumentów bazowych to **edycja w miejscu** dla stopniowego udoskonalania, **archiwizacja, a następnie zastąpienie** dla pełnej regeneracji. Ta umiejętność tworzy *pełną* mapę drogową z PRD; chirurgiczne udoskonalanie jest poza zakresem. Domyślnie archiwizuj, a następnie zastąp, ale zapytaj za pomocą wybranego narzędzia do zadawania pytań interaktywnych:
...
- Nadpisz bez archiwizacji
  Opis: Zastąp w miejscu. Istniejąca zawartość zostanie utracona (chyba że ją zatwierdziłeś). Używaj tylko wtedy, gdy istniejąca mapa drogowa jest pusta lub robocza.
- Anuluj
...
═══════════════════════════════════════════════════════════
  ...
  Projekt:           <projekt>
  Kamień milowy:         M-<seq>: <nazwa>  (<milestone_id>)  —  open
...
3. W przeciwnym razie, jeśli żaden fragment nie jest `ready`, zarekomenduj rozwiązanie pytania otwartego lub blokera o największym wpływie (tego, który odblokowuje najwięcej elementów niższego szczebla). Do tego czasu nie ma dostępnego ruchu planistycznego.
4. W przeciwnym razie zarekomenduj `ready` fragment, który odblokowuje najwięcej elementów niższego szczebla (największe rozgałęzienie w grafie zależności). Rozstrzygnij remisy według głównego celu (Krok 6d).
...
► **Twój następny ruch:** `/10x-plan <change-id>` na **<ID mapy drogowej>: <Wynik>**.
  ...
  Zablokowane — pozostań zaparkowany, dopóki ich niewiadome nie zostaną rozwiązane:
...
1. **Materiały źródłowe są źródłem.** Każdy fragment odnosi się do identyfikatora kotwicy źródłowej — identyfikatorów PRD (`FR-NNN`/`US-NN`) dla kamieni milowych pochodzących z PRD, kotwic karty `MS-NN` dla kamieni milowych pochodzących z opisu. Ramowanie z Kroku 5 ujawnia kontekst celu/gwiazdy północnej/inwestycji/blokera wywnioskowany ze źródeł; baza ujawnia to, co już istnieje; żadne z nich nie rozszerza źródeł. Elementy mapy drogowej bez śladu źródła są błędem samokontroli.
...
5. **Brak szacunków, brak jednostek czasu.** Bez "Dzień 1", bez "2 tygodnie", bez "mały/średni/duży", bez punktów. Wykonanie agenta AI jest nieliniowe, a szacunki budżetowane czasowo kłamią. Kolejność jest zakodowana w wymaganiach wstępnych; tempo ujawnia się poprzez blokery i niewiadome. Mapa drogowa opisuje kształt, a nie harmonogram.
...
9. **Samokontrola przerywa w przypadku dryfu.** Brak wymaganych sekcji, uszkodzony graf zależności, niepokryte FR typu must-have, wymyślone fragmenty, zbyt duże fragmenty, ukończenie warstwy fundamentu, sprzeczności między bazą a fundamentami — wszystko to przerywa zapis z konkretnym błędem. Bez cichej poprawki.
...
11. **Tylko język uniwersalny.** Bez odniesień do 10xDevs / kohorty / certyfikacji w żadnym wyjściu widocznym dla użytkownika ani w żadnym artefakcie zapisanym na dysku. Umiejętność jest ogólnym generatorem map drogowych.
