...
- `/10x-implement` (i jego autonomiczny odpowiednik `/10x-goal-implement`) — dalej w dół. Kiedy implementacja *rozpoczyna się* dla zmiany, której `Change ID` pasuje do elementu mapy drogowej, zmienia `Status` tego elementu na `in-progress` — odpowiednik `done` z `/10x-archive` dla pracy w toku. Ta umiejętność sama w sobie nadal emituje tylko `proposed` / `ready` / `blocked` podczas generowania; pośrednie stany cyklu życia (`planning`, `in-progress`) są teraz zapisywane w dół, gdy zmiana przechodzi przez plan → implementację. Każde przejście w dół pasuje do `Change ID`, jest najlepszym wysiłkiem (brak dopasowania to ciche pominięcie) i jest tylko do przodu (nigdy nie cofa bardziej zaawansowanego statusu).
...
Zawsze, gdy procedura mówi o użyciu subagentów lub uruchomieniu równoległych sond, użyj dowolnego narzędzia do badań w tle / tworzenia zadań, które udostępnia host (na innych hostach, dowolne narzędzie, które tworzy izolowanego agenta i zwraca podsumowanie), rozsyłając sondy w jednym wywołaniu wsadowym. Jeśli żadne nie istnieje, uruchom te same sondy sekwencyjnie w głównym kontekście. Obie ścieżki muszą zwrócić ten sam kształt podsumowania bazowego z dowodami plików.
...
- Inne dokumenty — podam ścieżki
  Opis: Specyfikacje, briefy, dokumenty badawcze. Fragmenty będą odnosić się do ich treści, zapisanych jako kotwice zakresu w karcie kamienia milowego.
- Sam opiszę kamień milowy
  Opis: Swobodny opis, bez dokumentu. Destyluję go do kotwic zakresu MS-NN, do których będą odnosić się fragmenty.
- Anuluj
...
Jeśli plik istnieje, **przeczytaj go W CAŁOŚCI** (bez `limit`/`offset`). Jeśli użytkownik wybrał samodzielny opis, przechwyć jego opis dosłownie — stanie się on kartą `## Milestone` z numerowanymi kotwicami zakresu `MS-NN`, a sprawdzenie gotowości PRD w Kroku 3 zostanie zastąpione sprawdzeniem kotwicy (< 2 kotwice `MS-NN` do destylacji → poproś użytkownika o doprecyzowanie opisu, a następnie ZATRZYMAJ, jeśli nie może).
...
1. **Wizja i Problem Statement są nietrywialne** — sekcja istnieje, zawiera ≥ 2 zdania, NIE zawiera `# TODO`.
...
4. **Business Logic wypełniona** — pierwsza niepusta linia sekcji `## Business Logic` jest zdaniem deklaratywnym (nie `# TODO: domain rule`).

Jawnie udokumentuj heurystykę w rozmowie:

```
Sprawdzenie gotowości PRD (heurystyka, 4 sygnały, po 1 punkcie):
  [✓|✗] Wizja i Problem Statement nietrywialne
  [✓|✗] ≥ 1 wypełniona historyjka użytkownika
  [✓|✗] ≥ 1 FR typu must-have
  [✓|✗] Business Logic wypełniona
...
To PRD uzyskało <N>/4 w heurystyce gotowości mapy drogowej. Brakujące sygnały:
  ...
  - <nazwa sygnału>: <jednoliniowa konsekwencja dla mapy drogowej>
  - ...
...
Opcje:
- Najpierw dopracuj PRD (Zalecane)
...
Ocena "co już jest na miejscu" nie powinna spadać na użytkownika — baza kodu jest źródłem prawdy. Użyj wybranego narzędzia do badań w tle / tworzenia zadań, jeśli jest dostępne, aby inwentaryzować każdą warstwę równolegle. Jeśli takie narzędzie nie istnieje, uruchom te same sondy sekwencyjnie w głównym kontekście. Każda sonda zwraca jednozdaniowy werdykt: **obecny** (z dowodami plików), **nieobecny** lub **częściowy** (szkielet istnieje, ale nie jest podłączony). Następnie przedstaw inwentaryzację do potwierdzenia przez użytkownika, zanim zostanie ona przekazana do Foundations.
...
Zapytaj użytkownika: "Czy ta baza odpowiada Twojemu rozumieniu? Coś do poprawienia lub dodania, zanim zostanie to wykorzystane w Foundations?"
Opcje:
...
- Popraw jedną lub więcej warstw — wyjaśnię
  Opis: Swobodna korekta. Ponownie zapiszę warstwę(y) przed kontynuowaniem.
- Dodaj coś, czego nie ma na liście
  Opis: Swobodny tekst. Rzeczy, które sondy przeoczyły (zaplanowane, ale nie podłączone, szkielet z innego repozytorium itp.).
...
PRD zawiera **produkt**. Baza (Krok 4) zawiera **to, co już istnieje**. Ten krok tworzy ramy mapy drogowej — `main_goal`, `north_star`, obszary inwestycji, `top_blocker` — poprzez ograniczony wywiad: maksymalnie **trzy pytania kotwiczące**, każde zawierające jedną silną **rekomendację** opartą na cytowanej linii artefaktu plus 1-2 alternatywy z jednoliniowym uzasadnieniem "dlaczego to też jest rozsądne". Użytkownik wybiera rekomendację, wybiera alternatywę lub swobodnie ją nadpisuje; obszary inwestycji są *wywodzone* z odpowiedzi, a nie pytane. Jest to złoty środek między dwoma trybami awarii, przez które przeszła umiejętność: **ciche automatyczne ramowanie** (podejmowanie kluczowych decyzji bez ludzkiej bramki) i **nieograniczone odkrywanie** (pytanie o to, na co artefakty już odpowiadają). Jeśli `shape-notes.md` zawierał blok `## Forward: technical-roadmap`, wykorzystaj go w rekomendacjach — nie wyciągaj ponownie treści, które użytkownik już tam zaparkował. Jeśli kotwica jest nadal nierozstrzygnięta po osiągnięciu limitu, **podejmij decyzję** używając rekomendacji, zapisz ją w frontmatterze z jednoliniowym uzasadnieniem i kontynuuj — użytkownik może ją nadpisać w dowolnym momencie.
...
- <Wartość rekomendowana> (Zalecane)
  Opis: <Jednoliniowe uzasadnienie, z cytatem/wskaźnikiem artefaktu, który uzasadnia rekomendację.>
- <Wartość alternatywna A>
  Opis: Rozsądne, gdy <jednoliniowy warunek, który artefakty częściowo wspierają>; wybierzesz to, gdy <konsekwencja sekwencjonowania/zakresu>.
- <Wartość alternatywna B>
  Opis: Rozsądne, gdy <jednoliniowy warunek>; wybierzesz to, gdy <konsekwencja>.
- Coś innego — wyjaśnię
  Opis: Swobodny tekst. Podaj wartość i powód; zapiszę oba i odpowiednio je uporządkuję.
...
- **Opcje gwiazdy północnej nazywają kandydatów na fragmenty, a nie abstrakcyjne wartości** — każda etykieta to `<kandydat US-NN> — <jednoliniowy wynik>`.
...
- **Cel sekwencjonowania: `<main_goal>`.** <Jednoliniowe uzasadnienie, łączące się z odpowiedzią użytkownika na kotwicę i wskaźnikiem artefaktu.>
- **Gwiazda przewodnia: `<S-NN candidate> — <Outcome>`.** <Jednoliniowe powiązanie tego fragmentu z głównym kryterium sukcesu lub najbardziej ryzykownym założeniem.>
- **Główne ryzyko / bloker: `<top_blocker>`.** <Jednoliniowe z konkretnym sygnałem — liczba otwartych pytań, nazwany dostawca, niedopasowanie terminu itp.>
- **Inwestycje: w `<layer>` głęboko; reszta lekko.** <Jednoliniowe — wywiedzione z main_goal + NFR + luki w bazach; nie pytane.>
...
Gdy użytkownik powie "go" lub pozostanie cichy po przekroczeniu następnego kroku, kontynuuj z zablokowanym ramowaniem. Nadpisania poszczególnych linii są akceptowane i ponownie zapisywane bez ponownego pytania o inne kotwice.

**5f. Wyjątek niestandardowego kształtu MVP.**

"Niestandardowy kształt MVP" to produkt, który nie pasuje do znanego wzorca: nie jest to pulpit SaaS, nie jest to aplikacja CRUD, nie jest to platforma treści, nie jest to oczywisty wrapper AI, nie jest to strona marketingowa. Sygnały: `## Vision` PRD opisuje nowatorską interakcję lub domenę; `## User Stories` nie grupują się wokół znanej encji (tworzenie/czytanie/aktualizowanie/usuwanie `<rzeczy>`); `tech-stack.md` deklaruje nieoczywiste narzędzia (silniki gier, mosty sprzętowe, wyspecjalizowane środowiska uruchomieniowe, nowatorskie kształty agentów); sformułowanie użytkownika podkreśla nową mechanikę, a nie znany wzorzec.

Gdy PRD wygląda na niestandardowo ukształtowane:

1. **Otwórz wywiad, ujawniając to** w wiadomości poprzedzającej pierwsze pytanie kotwiczące: *"To PRD nie pasuje do znanego wzorca MVP (brak pulpitu SaaS / CRUD / treści / kształtu wrappera AI). Moje rekomendacje dla kolejnych 2-3 pytań są słabsze niż zwykle — mocno się sprzeciw, jeśli moja interpretacja jest błędna."*
...
- `## Non-Functional Requirements` PRD, które wymagają infrastruktury (np. NFR "p95 < 800ms" implikuje podstawowe instrumentowanie wydajności).
- `## Access Control` PRD, jeśli jest czymś więcej niż "pojedynczy użytkownik, brak uwierzytelniania".
- **Baza z Kroku 4** — wszystko, co zgłoszono jako **nieobecne** lub **częściowe**, jest kandydatem na Foundations. Wszystko, co zgłoszono jako **obecne**, jest pomijane (i odnotowane w `## Baseline`).
...
Identyfikatory Foundation to `F-NN` (dwucyfrowe z zerami wiodącymi, zaczynając od `F-01`).
...
Dla każdego fundamentu zidentyfikuj również **Unlocks**:

- jeden lub więcej identyfikatorów pionowych fragmentów `S-NN` niższego szczebla, które fundament bezpośrednio umożliwia, LUB
...
Jeśli fundament nie ma wyraźnych Unlocks, usuń go lub włącz pracę do pierwszego pionowego fragmentu, który go potrzebuje.

Następnie dla każdego elementu wywnioskuj **Parallel with** — fragmenty, których wymagania wstępne są podzbiorem lub rodzeństwem wymagań wstępnych tego fragmentu i które od niego nie zależą. Agenci AI mogą rozdzielać się na te fragmenty. Jeśli dwa fragmenty nie mają żadnych wspólnych zależności i żaden nie blokuje drugiego, są równoległe. Gdy głównym blokerem (Krok 5) jest **capacity**, bądź szczególnie hojny w obliczaniu parallel-with — jest to najbardziej użyteczna dźwignia dla użytkownika.

**6d. Sortowanie topologiczne, z uwzględnieniem głównego celu.** Najpierw Foundations (w kolejności zależności między nimi), następnie fragmenty w kolejności zależności. Umieść fragment **north star** tak wcześnie, jak pozwalają na to jego wymagania wstępne — nie odkładaj go dla symetrycznego porządku. Następnie rozstrzygnij remisy według głównego celu (Krok 5):
...
- **Quality / craft** → Foundations sekwencjonowane bardziej chętnie; fundamenty obserwowalności i kontroli dostępu NIE są odkładane za fragmenty widoczne dla użytkownika.
...
- **Learn the tech / explore** → remisy rozstrzygane na korzyść fragmentów, które najwcześniej wykorzystują nieznaną technologię; wartość uczenia się liczy się tutaj jako wartość dla użytkownika.
...
- **Blokery** (zewnętrzne, oczekujące) — zatwierdzenie dostawcy, zasób projektowy, decyzja interesariusza. Jeśli brak, napisz `—`. Odpowiedź na pytanie "External" #1-bloker z Kroku 5 zasila te blokery.
- **Niewiadome** (pytania do zbadania) — rzeczy, na które mapa drogowa nie może odpowiedzieć, a `/10x-plan` również nie powinien próbować. Każda niewiadoma zawiera: pytanie, właściciela, status blokowania (tak/nie — czy planowanie jest zablokowane do czasu rozwiązania tego problemu?). Odpowiedź na pytanie "Decisions" #1-bloker z Kroku 5 zasila te niewiadome.
...
**6g. Wygeneruj `## Parked`.** Podnieś `## Non-Goals` z PRD. Dodaj również wszystko, co Krok 5 ujawnił jako odłożone — szczególnie gdy głównym celem jest **szybkość uruchomienia** lub głównym blokerem jest **czas/pojemność**, ta sekcja rośnie. Każdy wpis: jednoliniowy element, jednoliniowe uzasadnienie.

**6h. Wywnioskuj `## Streams` (pomoc nawigacyjna).** Strumienie to *widok pochodny* grafu zależności — NIE zastępują one porządku topologicznego w `## Foundations` + `## Slices` i nie wprowadzają nowych identyfikatorów. Ich zadanie: przedstawić czytelnikowi proponowaną kolejność czytania na równoległych ścieżkach na jednym ekranie. Wyprowadzenie: jeden strumień na fundament, który kotwiczy odrębny łańcuch wymagań wstępnych (`F-NN` → fragmenty wymieniające go w wymaganiach wstępnych, w kolejności zależności); fragment bez wymagań wstępnych fundamentu jest własnym strumieniem jednopozycyjnym (nigdy nie jest to "Misc" bucket); fragment zależny od wielu głów strumieni dołącza do najbardziej pochodnego, z nazwą połączenia w notatce tego strumienia ("łączy się ze strumieniem A w S-01") — nigdy nie jest duplikowany w strumieniach. Wygeneruj jeden wiersz tabeli markdown na strumień — `Stream | Theme | Chain | Note` — Chain łączący istniejące identyfikatory mapy drogowej za pomocą `→`, Theme opisowy, a nie promocyjny ("Pętla przeglądu", a nie "Zabójcza funkcja"), Note jedna klauzula łącząca strumień z `main_goal` lub nazywająca połączenie. Limit: 2-5 strumieni — więcej oznacza, że graf jest zbyt mocno podzielony (zwiń strumienie jednopozycyjne w strumień sąsiedniego fundamentu); mniej niż 2 oznacza, że porządek topologiczny jest już czytelny, więc pomiń sekcję. Strumienie NIE są kanoniczne: w przypadku jakiegokolwiek konfliktu, porządek topologiczny wygrywa, a definicja strumienia jest błędna.
...
zdaniu w prostym języku. Przykład: "Klin produktu — jedyna cecha, która,
gdyby została usunięta, sprawiłaby, że produkt byłby nie do odróżnienia od
ogólnego narzędzia AI — polega na tym, że karty muszą być zarówno oparte na
AI w tekście wklejonym przez uczącego się, jak i zatwierdzone przez człowieka,
...
> Jednoliniowy opis dla czytelnika, wyjaśniający, co oznacza "gwiazda północna":
...
> działa. Włącz to wyjaśnienie za PIERWSZYM razem, gdy "gwiazda północna"
...
|---|---|---|---|
| A | <Temat> | `F-01` → `S-01` → `S-02` | <Jednoliniowe uzasadnienie łączące strumień z głównym celem.> |
...
- **Change ID:** <kebab-case-change-id>
- **Odnośniki PRD:** <FR-NNN, US-NN, NFR-N — każdy FR typu must-have, który ten fragment spełnia, każdy US-NN, który rozwija>
...
Ta tabela to czyste przekazanie do Jira/Linear lub dowolnego backlogu wspieranego przez MCP. Uwzględnij jeden wiersz dla każdego `F-NN` i `S-NN`. Powinna być wystarczająco kompaktowa, aby skopiować ją do zadań, ale nie może duplikować szczegółowej treści mapy drogowej.
...
(Tylko do dopisywania. Przenoszone dosłownie do mapy drogowej każdego kolejnego kamienia milowego; puste przy pierwszym kamieniu milowym. Wpisy zamknięcia są zapisywane przez przejście `READY_TO_CLOSE → CLOSED` tej umiejętności. Format:)

- **M-<seq>: <Nazwa kamienia milowego>** (`<milestone_id>`) — zamknięty <RRRR-MM-DD>. <Jednoliniowy wynik.>
...
- **Wynik** jest prowadzony czasownikiem. Fragmenty: *"użytkownik może się zalogować i zobaczyć pustą lodówkę"*. Fundamenty: *"(fundament) szkielet uwierzytelniania wylądował; tokeny wydane za pośrednictwem skonfigurowanego dostawcy"*. Nigdy fraza rzeczownikowa ("system uwierzytelniania"); zawsze deklaratywny stan świata.
...
- **Unlocks** pojawia się tylko w Foundations. Nazywa powód niższego szczebla, dla którego ten Fundament istnieje: konkretne fragmenty `S-NN`, blokujące niewiadome lub ścieżki weryfikacji. Fundament bez Unlocks to dryf poziomy.
- **Odnośniki PRD** używają dosłownych identyfikatorów z PRD (`FR-001`, `US-01`, `NFR-02`). Nie parafrazuj. Każdy FR typu must-have w PRD musi pojawić się w `PRD refs` co najmniej jednego fragmentu po samokontroli w Kroku 8.
...
- **Niewiadome** to pytania do zbadania. Każde zawiera właściciela i flagę blokowania. Block=yes zmienia status fragmentu na `blocked`.
- **Ryzyko** to jedna linia: dlaczego sekwencjonowane tutaj, co może pójść nie tak, dlaczego jest to bezpieczniejsza kolejność niż alternatywy. Nie jest to analiza pośmiertna. Nie jest to katastrofizowanie. Tylko kluczowy powód, który przyszły czytelnik musi zrozumieć, aby zrozumieć sekwencję.
- **Status** cykl życia: `proposed` (domyślny przy pierwszym generowaniu) | `ready` (wszystkie wymagania wstępne spełnione, brak blokujących niewiadomych — `/10x-plan` może działać) | `planning` | `in-progress` | `done` | `blocked` (jedna lub więcej niewiadomych z `Block: yes`). Ta umiejętność emituje tylko `proposed`, `ready` i `blocked` podczas generowania; reszta jest zapisywana w dół (patrz "Relacja do innych umiejętności"), z najlepszym wysiłkiem i tylko do przodu.
...
1. **Frontmatter** — wszystkie 11 kluczy obecne (`project`, `version`, `status`, `created`, `updated`, `prd_version`, `main_goal`, `top_blocker`, `milestone_id`, `milestone_seq`, `milestone_status`).
...
8. **Brak wymyślonych fragmentów** — `PRD refs` każdego fragmentu zawiera co najmniej jeden rzeczywisty identyfikator kotwicy źródłowej: identyfikatory PRD (`FR-\d{3}` lub `US-\d{2}`) dla kamieni milowych pochodzących z PRD, lub kotwice karty `MS-\d{2}` dla kamieni milowych pochodzących z opisu. Mieszane źródła mogą mieszać typy identyfikatorów, ale każda kotwica musi istnieć w dokumencie źródłowym lub w karcie `## Milestone`.
...
10. **Umowa o włączaniu fundamentów** — każdy Fundament ma `Unlocks` wypełnione co najmniej jednym fragmentem `S-NN` niższego szczebla, nazwaną blokującą niewiadomą lub nazwaną ścieżką weryfikacji. Ogólny fundament, taki jak "warstwa bazy danych" bez powodu niższego szczebla, jest błędem samokontroli.
...
16. **Integralność kamienia milowego** — `milestone_status` jest `open` podczas generowania; `milestone_seq` jest o 1 większe niż najwyższe zamknięte `M-<seq>` w `## Milestone History` (1, gdy historia jest pusta); karta `## Milestone` `M-<seq>` odpowiada `milestone_seq`; każda kotwica `MS-NN` odwołująca się do dowolnego fragmentu istnieje w karcie; `## Milestone History` została przeniesiona dosłownie (nigdy nie edytowana, nigdy nie obcinana) podczas regeneracji i otwierania następnego kamienia milowego.
...
Jeśli plik istnieje, konwencja dokumentów bazowych to **edycja na miejscu** dla stopniowego udoskonalania, **archiwizacja, a następnie zastąpienie** dla pełnej regeneracji. Ta umiejętność tworzy *pełną* mapę drogową z PRD; chirurgiczne udoskonalanie jest poza zakresem. Domyślnie archiwizuj, a następnie zastąp, ale zapytaj za pomocą wybranego narzędzia do interaktywnych pytań:

Zapytaj użytkownika: "context/foundation/roadmap.md już istnieje. Jak chcesz postąpić?"
Opcje:
- Archiwizuj i zastąp (Zalecane)
  Opis: Przenieś istniejący plik do context/foundation/archive/<dzisiaj>-roadmap.md, a następnie zapisz nową mapę drogową. Historia zachowana zgodnie z konwencją README fundamentu.
...
3. W przeciwnym razie, jeśli żaden fragment nie jest `ready`, zarekomenduj rozwiązanie najbardziej wpływowego otwartego pytania lub blokera (tego, który odblokowuje najwięcej elementów niższego szczebla). Do tego czasu nie ma dostępnego ruchu planistycznego.
4. W przeciwnym razie zarekomenduj `ready` fragment, który odblokowuje najwięcej elementów niższego szczebla (najwyższy fan-out w grafie zależności). Rozstrzygnij remisy według głównego celu (Krok 6d).
...
ZATRZYMAJ. Nie łącz automatycznie z inną umiejętnością — użytkownik wybiera, kiedy planować. Ale NIE obniżaj rekomendacji do listy wielokrotnego wyboru; jeśli użytkownik chce innego fragmentu, mówi o tym.
...
6. **Brak niskopoziomowych szczegółów technicznych.** Brak nazw frameworków (te znajdują się w `tech-stack.md`), brak ścieżek plików, brak definicji schematów, brak kodu, brak wyborów bibliotek. Jeśli zauważysz, że piszesz takie rzeczy, przekroczyłeś terytorium `/10x-plan` — zatrzymaj się i pozwól `/10x-plan` wykonać swoją pracę w dół.

7. **Ujawnij niewiadome, nie tuszuj ich.** Niewiadome dotyczące poszczególnych fragmentów z `Block: yes` zmieniają status na `Status: blocked`. Przekrojowe niewiadome trafiają do `## Open Roadmap Questions`. Jeśli PRD ma TODO, mapa drogowa dziedziczy je jako niewiadome zablokowanych fragmentów. Wartość mapy drogowej polega częściowo na pokazywaniu użytkownikowi, co JESZCZE nie jest możliwe do zaplanowania.
...
9. **Samokontrola przerywa w przypadku dryfu.** Brak wymaganych sekcji, uszkodzony graf zależności, niepokryte FR typu must-have, wymyślone fragmenty, zbyt duże fragmenty, ukończenie warstwy Fundamentu, sprzeczności między Baseline a Foundations — wszystko to przerywa zapis z konkretnym błędem. Brak cichej poprawki.
...
14. **Oszczędny wywiad z silnymi rekomendacjami — bez cichego automatycznego ramowania, bez nieograniczonego odkrywania.** Zasady z Kroku 5 są normatywne: maksymalnie 3 pytania kotwiczące (`main_goal`, `north_star`, `top_blocker`), obszary inwestycji wywodzone, a nie pytane, każde pytanie jedna rekomendacja oparta na cytowanej linii artefaktu plus 1-2 rzeczywiste alternatywy (słomiane kukły zabronione), pominięcia tylko wtedy, gdy artefakty dosłownie stwierdzają wartość, dalsze pytania tylko w ramach wyjątku niestandardowego MVP (5f). Zalecany następny ruch z Kroku 10 to ta sama zasada zastosowana do przekazania: jedna rekomendacja z jednoliniowym uzasadnieniem, a nie lista "gotowych do planowania", którą użytkownik musi ocenić.

15. **Kamienie milowe zapętlają się, ale nigdy nie są ograniczone czasowo.** Dokładnie jeden kamień milowy otwarty w danym momencie; zamyka się tylko wtedy, gdy każdy F-NN/S-NN jest `done` (lub użytkownik jawnie go porzuca), a następnie pętla otwiera się ponownie z nowymi materiałami źródłowymi lub opisem użytkownika. Stan kamienia milowego jest wywodzony wyłącznie z `roadmap.md` — brak plików bocznych. Specyfikacja cyklu życia znajduje się w `references/milestone-state.md`, ładowana TYLKO dla operacji na poziomie kamienia milowego (rozsyłanie w Kroku 0). Umiejętności niższego szczebla pozostają ślepe na kamienie milowe; ta umiejętność wykrywa ukończenie kamienia milowego przy następnym wywołaniu.
